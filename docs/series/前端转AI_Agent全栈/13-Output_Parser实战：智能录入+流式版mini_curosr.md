...
还有一点要补充：前面讲 withStructuredOutput 底层会根据模型能力选择是 tool 还是 output parser，其实还有一种特性 JSON Schema
...
也就是说，withStructuredOutput 底层会根据模型能力选择是 tool、json schema 还是 output parser。
