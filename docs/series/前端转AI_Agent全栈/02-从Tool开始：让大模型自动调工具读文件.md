...
![image-20260206215939387](https://img.xiaojunnan.cn/02-image-20260206215939387.png)

根据 tool_calls 的数组，分别从 tools 数组里找到对应的工具，取出来 invoke，传入大模型解析出的参数，最后把工具调用结果作为 ToolMessage 传给大模型，让它继续回答：

![image-20260206220304217](https://img.xiaojunnan.cn/02-image-20260206220304217.png)
...
当模型推理出：“用户要求读取文件”，会返回一个结构化指令第二个对象就叫 tool call：
...
tool_calls = 一轮回复里所有 tool call 的集合，类型是数组为什么是数组？因为模型可能一次请求多个工具：
...
while (response.tool_calls && response.tool_calls.length > 0) {
  ...
  // toolResults就是我们tool执行后return的数据，读取文件就是文件内容
  // 将工具结果添加到消息历史
...
### 为什么进行了两次 await modelWithTools.invoke(messages)

因为工具调用是两段式的：

第 1 次 invoke（让模型“决定要不要用工具”）

模型看到用户问题后，可能会返回一个普通回答；也可能返回 response.tool_calls（告诉你：请调用哪些工具、参数是什么）。注意：此时模型还拿不到真实文件内容/工具结果，它只是提出“要调用工具”。执行工具（你这段代码里手动 tool.invoke）

你根据 response.tool_calls 真正去跑工具；把每个工具的结果包装成 ToolMessage 塞回 messages。第 2 次 invoke（把工具结果喂回模型，让它“基于结果回答”）

这次模型才看得到工具返回的“文件内容/结果”，才能做总结、解释、最终回复。

### 那为什么第一次response.content是空的

原因：第一次返回的是“工具调用指令”，不是“自然语言回答”在 LangChain 的工具调用协议里，模型如果决定要用工具，往往会返回一个 AIMessage，其核心信息在 response.tool_calls 里，而 response.content 可能就是 空字符串 ""（有的模型会给一句很短的过渡话，但也经常是空）。

所以你第一次 console.log(response.content) 看到空，并不代表没返回内容，而是返回内容主要长这样

response.tool_calls：要调用哪个工具、参数是什么、tool_call_id 是多少 response.content：可能为空（很常见）等你把工具结果以 ToolMessage 放回 messages 后，第二次 invoke 模型才有“材料”，才会在 response.content 里产出真正的总结/解释文本。
