...
> 库
>
> 也就是说：能拆得尽可能细。这样建立索引的时候，能够保存更多的关键词。

用 ik_smart 来做检索的分词

> `ik_smart` 是 IK 的**粗粒度分词模式**。
>
> 例如：人工智能知识库
>
> 可能拆成：
...
BM25 可以理解成 **Elasticsearch 里最经典的一种“关键词相关性打分算法”**。

简单说：

> **用户搜索一句话，BM25 负责判断：哪些文档和这句话最相关，并给每个文档打分。**
...
```
文档 A：Elasticsearch 是一个搜索引擎，支持 BM25 检索。


文档 B：Redis 是一个高性能内存数据库。


文档 C：Elasticsearch 可以使用 BM25 算法进行全文检索。
```
...
```
文档 A：Elasticsearch 是搜索引擎。


文档 B：Elasticsearch 是搜索引擎。Elasticsearch 支持 BM25，Elasticsearch 还支持向量搜索。
```
...
B 出现次数更多，所以通常会得到更高的相关性分数。

但 BM25 **不会简单地认为出现 10 次就一定比出现 1 次高很多**，它会对词频增长进行“饱和”，避免关键词堆砌。
...
虽然两篇都出现了关键词，但 A 更集中、更可能相关。

BM25 会考虑**文档长度**，避免长文档因为“天然包含更多词”而占优势。
...
如果整个知识库里 90% 的文档都有 `Elasticsearch`，这个词区分度就比较低。
