...
- **总结**：Hook是React 16.8的新增特性，它可以让我们在不编写class的情况下使用state以及其他的React特性(比如生命周期)【在不编写类组件的情况下】
...
在一个组件中，允许我们存在多个`useEffect`，他会从上之下依次执行
...
- ```jsx
  const tabClickHandle = useCallback(function(){
      console.log("ddd");
    },[])
  ```
...
类似于useCallback，但是他缓存的时候缓存的不是一个函数，而是函数返回的值
